...
def extract(a: torch.Tensor, t: torch.Tensor, x_shape: List) -> torch.Tensor:
    b, *_ = t.shape
    out = a.gather(-1, t)
    return out.reshape(b, *((1,) * (len(x_shape) - 1)))


def noise_like(shape: List, device: str, repeat: bool=False) -> torch.Tensor:
...
class GaussianDiffusion(torch.nn.Module):
    ...
    def compute_loss(self, x, *args, **kwargs):
        ...
        t = torch.randint(0, self.num_timesteps, (len(x),), device=x.device).long()
